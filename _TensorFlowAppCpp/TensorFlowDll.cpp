...
g++ -shared -std=c++11 -Wall -Wextra -I"include" TensorFlowDll.cpp -o TensorFlowApp64.dll -L"lib" -l:tensorflow.lib
...
// Companion function to free the allocated memory
extern "C" __declspec(dllexport) void __cdecl FreeString(const char* str) {
    if (str) {
        std::free(const_cast<char*>(str)); // Free the allocated memory
    }
}